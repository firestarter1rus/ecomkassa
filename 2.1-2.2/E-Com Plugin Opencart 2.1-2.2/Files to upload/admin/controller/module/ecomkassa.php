...
class ControllerModuleEcomkassa extends Controller { 
	...
	public function install() {
		$this->load->model('module/ecomkassa');
		$this->load->model('setting/store');
		$this->model_module_ecomkassa->install();
	}
	
	public function uninstall() {
		$this->load->model('setting/store');
        $this->load->model('module/ecomkassa');
        $this->model_module_ecomkassa->uninstall();
	}
	...
	public function index() {
		...
		$data['check_url'] = $this->url->link('module/ecomkassa/check', 'token=' . $this->session->data['token'], 'SSL');
		...
		$data['cron'] =  $url->link('module/ecomkassa/cron' );
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('module/ecomkassa', 'token=' . $this->session->data['token'], 'SSL')
		);

		$data['action'] = $this->url->link('module/ecomkassa', 'token=' . $this->session->data['token'], 'SSL');
		...
		$results = $this->model_module_ecomkassa->getExtensions('payment');
		...
		$this->response->setOutput($this->load->view('module/ecomkassa.tpl', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/ecomkassa')) {
	...
	*/
	public function check(){
		...
		$this->load->model('module/ecomkassa');
		$check = $this->model_module_ecomkassa->getCheck($login, $pass, $url);
	...
	*/
	public function sell(){
		...
		$this->load->model('module/ecomkassa');
		$receipt = $this->model_module_ecomkassa->getOrderReceipt($order_id);
		if(!empty($receipt)){
		...
		$authToken = $this->model_module_ecomkassa->getToken();
		if(!$authToken){
		...
		$result = $this->model_module_ecomkassa->sell($order_info, $authToken);
	...
	*/
	public function sellRefund(){
		...
		$this->load->model('module/ecomkassa');
		$receipt = $this->model_module_ecomkassa->getOrderReceipt($order_id, 'refund');
		if(!empty($receipt)){
		...
		$receipt = $this->model_module_ecomkassa->getOrderReceipt($order_id, 'sell');
		if(empty($receipt)){
		...
		$result = $this->model_module_ecomkassa->refund($order_info,$receipt, $authToken);
	...
	*/
	public function sellCorrection(){
		...
		$result = $this->model_module_ecomkassa->correction($order_info,$receipt, $authToken);
		echo $result;
	}
