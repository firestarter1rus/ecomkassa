...
class ControllerExtensionModuleEcomkassa extends Controller {
	...
	public function callback() {
		...
		if($json->status == 'done' || $json->status == 'fail'){
			...
			$uuid =  $json->uuid;
			$receipt =  $this->model_extension_payment_ecomkassa->loadReceipt($uuid );
			if(empty($receipt)){
				return;
			}
			...
			$this->model_extension_payment_ecomkassa->updateOrderReceipt($receipt);
		}
	}
	...
	public function cron() {
		...
		$this->load->model('extension/payment/ecomkassa');
		$receipts = $this->model_extension_payment_ecomkassa->getWaiting();
		var_dump(	$receipts );
		foreach($receipts as $receipt){
			echo $receipt['uuid'];
			$this->model_extension_payment_ecomkassa->getUpdate($receipt);
		}
