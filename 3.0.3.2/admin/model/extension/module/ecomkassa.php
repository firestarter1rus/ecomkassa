...
class ModelExtensionModuleEcomkassa extends Model {
	...
	public function addOrderReceipt($order_info,$request, $json, $type='sell'){
				...
				$this->db->query("INSERT INTO " . DB_PREFIX . "ecomkassa SET 
						...
						sno = '" .$this->db->escape($this->config->get('module_ecomkassa_sno')). "' 
						");
	}
	...
	public function refund($order_info,$receipt, $authToken){
		...
		$url =trim(  $this->config->get('module_ecomkassa_url'), '/').'/'.$shop_id. '/sell_refund?tokenid='.$authToken;  
	...
	public function correction($order_info,$receipt, $authToken){
		...
		$url =trim(  $this->config->get('module_ecomkassa_url'), '/').'/'.$shop_id. '/sell_correction?tokenid='.$authToken;  
		$order_products = $this->getOrderProducts($order_info['order_id']);
			...
			if(!empty($this->config->get('module_ecomkassa_sno'))){
			$request['correction']['attributes']['sno'] = $this->config->get('module_ecomkassa_sno');      
			 }
			 
			 $request['correction']['attributes']['tax']  = $this->config->get('module_ecomkassa_vat');      
	...
	public function getCheck($login, $pass, $url){
		...
		if(!$this->isJson($response)){
			$jsondata['code']=101;
			$jsondata['text']='Ошибка Login или Password';
